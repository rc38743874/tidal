					...
					while (reader.Read()) {
						...
						if (validNameList.Contains(fieldName) == false) {
							...
							try {
								...
								Shared.Info("last call to ConvertSQLToCSharp was for procedure " + procedureDef.ProcedureName + ", field named \"" + fieldName + "\".  This can happen if you have a stored procedure that is referencing a column that no longer exists for a table.  Check that the procedure will run on its own.");
								throw;
							}
