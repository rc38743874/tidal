...
namespace TidalCSharp {
	public class MySQLClassCreator : IClassCreator {
		...
		public string GetDataAccessClassText(string projectNamespace, string modelNamespace, List<ModelDef> modelDefList) {
			...
			buildText.AppendLine("\t\tpublic static MySqlCommand GetCommand(MySqlConnection conn, string procedureName) {");
			buildText.AppendLine("\t\t\treturn new MySqlCommand(procedureName, conn) { CommandType = CommandType.StoredProcedure };");
			buildText.AppendLine("\t\t}");
			buildText.AppendLine("");
			buildText.AppendLine("");
		...
		public void AddFunctionDeclaration(StringBuilder buildText, FunctionDef functionDef) {
			...
			buildText.Append("\t\t\tpublic static " + code + " " + functionDef.FunctionName + "(MySqlConnection conn");
		}
	...
	//		foreach (ArgumentDef argumentDef in functionDef.ArgumentDefList) {
				...
				if (parameterDef.IsOutParameter == true) {
					buildText.AppendLine("\t\t\t\t\tvar outParameter = new MySqlParameter { ParameterName = \"" + parameterDef.ParameterName + "\", Direction = ParameterDirection.Output, Size = " + parameterDef.ParameterSize + ", MySqlDbType = MySqlDbType." + GetMySqlDbTypeCode(parameterDef.ParameterDataTypeCode) + " };"); /* SqlDbType.Int */
		...
		public void AddBuildList(StringBuilder buildText, ModelDef modelDef) {
			buildText.AppendLine("\t\t\tprivate static List<Models." + modelDef.ModelName + "> BuildList(MySqlCommand command) {");
		...
		private string GetMySqlDbTypeCode (string inputCode) {
			switch (inputCode) {
				case "bigint":
					return "Int64";
				case "int":
